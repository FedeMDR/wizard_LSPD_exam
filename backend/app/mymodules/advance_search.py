...
class GeoCoords:
    ...
    def __init__(self, latitude : float, longitude : float):
        ...
        Raises:
            ValueError: If the provided latitude or longitude values are outside the valid ranges.
        '''
...
def calculate_center(selected_attractions : str):
    '''
    Calucalte the mean latitude and mean longitude of a given list of coordinates, to compute the center point.

    Arg: 
        selected_attraction (str): a list of selected attractions, subset of the attractions in Location.csv.
        
    Returns: 
        GeoCoords object : latitude and longitude of the center point stored in a GeoCoords object.
    '''
    df = ATTRACTION
...
def is_within_range(lat : float, lon : float, center : GeoCoords, range : float):
    ...
    '''
    Checks if a given set of latitude and longitude is inside a given range from a given center

    Args:
        lat (float) : latitude of the location we want to check.
        lon (float) : longitude of the location we want to check.
        center (GeoCoords object) : the geographical coordinates of the central point of our search_area 
                                    returned by the function "calculate_center".
...
def dist_to_deg(dist : int, center : GeoCoords):
    '''
    Compute the degree variation in latitude that correspond to the meters inputted. 
    This result will be used as the search_range in the function "find_airbnb_in_range".

    Args:
        dist (int) : distance in meters that correspond to the radius of the search_area.
        center (GeoCoords object) : the geographical coordinates of the central point of our search_area returned 
                                    by the function "calculate_center".
    
    Returns:
        float : the degree variation in latitude converted from the meters inputed.
    '''
...
def find_airbnb_in_range(center : GeoCoords, range : float):
    ...
    Returns:
        pd.DataFrame : A pandas dataframe containing only the Airbnb whose coordinates fall inside the given range.  
    '''
...
def get_attractions_list(): 
    '''
    extract the list of the names for each attraction in the attractions dataframe ('Location.csv')
