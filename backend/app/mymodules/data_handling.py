...
def corrZipAtt(min : int, max : int):
    '''
    Count the number of attractions for each zipcodes and filters the zipcodes that have at leat {min} attractions and at most {max} attractions.
    ...
    Returns:
        list : list of the zipcodes that contains at least the number of attraction that the user want to see.
    '''
...
def corrZipTrees(trees_bool : str):
    '''
    ...
    Returns:
        list : 
                if True : all the zipcodes that have a number of trees that is above the mean of NYC.
...
def corrZipCrime(crime_rate : int):
    '''
    Computes the crime rate threshold for each level of crime (from 1 to 4)
    
    Args:
        crime_rate (int) : a value from 1 to 4 given by the user depending on how much he cares about crimes.
    
    Returns: 
        list : a list of the zipcodes that comply with the crime level selected
    '''
    if(crime_rate == 4):
...
def BnbPerZip(zip_list : list, bnb_df : pd.DataFrame):
    ...
    Args:
        ...
        bnb_df (pd.DataFrame) : A Dataframe containing at least airbnbs and their zipcodes.

    Returns:
        pandas DataFrame : a subset of our original dataframe containing only the selected airbnbs
    '''
...
def get_bnb_by_neighborhood(target_neighbourhood : str):
    ...
    Raises:
        ValueError : if the target neighbourhood is empty or is not a neighbourhood of NYC.
    
    Returns:
        padas DataFrame : the first 50 airbnbs located inside the given neighbourhood.

    '''
    if target_neighbourhood not in NEIGHBOURHOODS:
        target_neighbourhood == None
    if not target_neighbourhood:
        raise ValueError("Input must be a string and must be a NYC neighbourhood.")
