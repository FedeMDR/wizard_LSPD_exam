...
class SeachBnBForm(FlaskForm):
    ...
    sorting_order = SelectField('Sorting Order:', choices=[(0, 'Ascending'),
                                                           (1, 'Descending')])
...
class AdvancedSearch(FlaskForm):
    selected_attractions = SelectMultipleField(choices=request_attr_list(),
                                               validators=[DataRequired()])
    distance_m = IntegerField('Distance (m)', validators=[NumberRange(min=0),
                                                          DataRequired()])
    sorting_key = SelectField('Sort by:', choices=[('price', 'Price'),
                                                   ('review_scores_rating', 'Reviews')])
    sorting_order = SelectField('Sorting Order:', choices=[(0, 'Ascending'),
    ...
    if form.validate_on_submit():
        ...
        if response.status_code == 200:
            ...
            if form.sorting_key.data == 'review_scores_rating':
                data = sorted(data, key=lambda x: int(x[form.sorting_key.data]*100),
                              reverse=bool(int(form.sorting_order.data)))
            else:
                data = sorted(data, key=lambda x: x[form.sorting_key.data],
                              reverse=bool(int(form.sorting_order.data)))
